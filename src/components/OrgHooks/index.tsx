...
const getModalTitle = (modal: string) => {
  switch (modal) {
    case 'edit':
      return {
        modalTitle: 'Edit',
        modalDescription: 'Edit hook details'
      }
    case 'create':
      return {
        modalTitle: 'Create',
        modalDescription: 'New hook'
      }
    default:
      return {
        modalTitle: '',
...
// Main page
// ========================================================
const OrgHooks = ({ orgId }: { orgId: string }) => {
  // State / Props
   ...
   * LIST
   */
  const { isLoading: isRetrieving, data: hooksListData, error: hooksListError, mutate: hooksList } = useMutation(HOOKS.LIST);
   ...
   * 
   */
  const onClickEditHook = (id: string) => () => {
  }

  /**
   * 
   */
  const onClickAddHook = () => { }

  // Render
  return <div>
    <div className="flex items-center justify-between mb-8">
      <Heading as="h4">Hooks</Heading>
      <Button onClick={() => { }}>Add Hook</Button>
    </div>
    <div className="relative block mb-8">
      <Search className="w-4 text-slate-400 absolute left-4 top-0 bottom-0 my-auto" />
      <Input value={search} onChange={onChangeSearch} disabled={isRetrieving} variant="none" className="w-full h-12 rounded-full pl-10 pr-6" type="search" placeholder="Search for hooks" />
    </div>
    <div className="w-full">
      {isRetrieving ? <div className={`rounded-lg bg-white border border-slate-200 p-8 transition-colors ease-in-out duration-200`}>
      ...
      </div> : hooksListData && hooksListData.length > 0 ?
        <div className="border border-slate-200 rounded-lg px-4">
          <table className="w-full mb-4">
            ...
            <tbody>
              {hooksListData.map((hook: any) =>
                <tr key={`${hook.id}`} onClick={onClickEditHook(hook.id)} className="border-b border-l-slate-200 hover:bg-slate-50 cursor-pointer">
                  <td className="p-4 w-3/4">
                    <div className="flex items-center">
                      ...
                      <code className="text-slate-500">{hook?.walletAddress}</code>
                    </div>
                  </td>
          ...
          <div className="rounded-lg bg-slate-100 py-16 lg:py-24 px-8 w-full">
            ...
            {!search ? <Button onClick={onClickAddHook} variant="gray" className="block mx-auto">Add Hook</Button> : null}
          </div>
        </div>}
    </div>
  </div>
